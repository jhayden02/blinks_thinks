...
{
    public:
    // Shader management class. Handles loading, applying, and unloading all game shaders.
    class shader_manager
    {
        ...
        // Begins rendering to an internal texture for shader processing.
        void begin();

        // Queues a shader to be applied during the next process() call.
        void append(string shader_name);

        // Applies all queued shaders in order and draws the final result to the screen.
        void process();

        private:
        // Primary render target for switching between shader passes.
        RenderTexture2D m_target_a;

        // Secondary render target for switching between shader passes.
        RenderTexture2D m_target_b;

        // Map of all loaded shaders, indexed by name.
        unordered_map<string, Shader> m_shaders;

        // Queue of shader names to be applied in the current frame.
        vector<string> m_shader_queue;

        // Tracks whether BeginTextureMode has been called without a matching EndTextureMode.
    ...
    class audio_manager
    {
        ...
        // The song that should be mixed in.
        Music* m_next_music;
        
        // The current pitch of the music.
        float m_current_pitch;
        
        // The pitch that should be shifted to.
        float m_next_pitch;
        
        // The current frame count of the mixing process.
        int m_frame_count_mix;
        
        // The current frame in the pitch shifting process.
        int m_frame_count_pitch;

        // If two songs are being mixed currently.
        bool m_mixing;
        
        // If the pitch is being shifted to something.
        bool m_shifting;
    };
    ...
    // The audio manager object for the game.
    audio_manager* audio;

    // The shader manager object for the game.
    shader_manager* shaders;

    // The main runtime method to start the game from 'main'.
    void run();
